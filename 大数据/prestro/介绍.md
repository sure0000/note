...
presto 不是用来替代 mysql/pg/oracle 的，presto 设计的目的也不是用来处理 OLTP 的，presto 设计的目的是为了通过分布式查询有效的查询大批量的数据，例如，查询TB或者PB级别的数据，是查询 HDFS、hive的一个可选项，但同时不局限于 HDFS, presto 适用于 OLAP 的场景。
